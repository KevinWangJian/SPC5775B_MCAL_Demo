 ...
 *     Autosar ConfVariant :
 *     SWVersion           : 1.0.1
 *     BuildVersion        : MPC5777C_MCAL_1_0_1_RTM_HF4_ASR_REL_4_0_REV_0003_20180627
 *
...
#define MCL_AR_RELEASE_MINOR_VERSION_C      0
#define MCL_AR_RELEASE_REVISION_VERSION_C   3
...
*/
/*
* @violates @ref Mcl_c_11 MISRa Rule 8.7 Objects %s shall be defined at block scope ..
*/
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
LOCAL_INLINE FUNC(uint32, MCL_CODE) Mcl_GetPhysicalAddress(VAR(uint32, AUTOMATIC) logicalAddr);
#endif /*MCL_LOGICAL_ADDRESS_USED == STD_ON*/
#endif /*defined MCL_LOGICAL_ADDRESS_USED*/
    
#endif /* #if (MCL_ENABLE_DMA == STD_ON) */
/*==================================================================================================
 *                                        LOCAL FUNCTIONS
==================================================================================================*/
                                        ...
                                        P2CONST(Mcl_ConfigType, AUTOMATIC, MCL_APPL_CONST) ConfigPtr)
{
    VAR(Std_ReturnType, AUTOMATIC) valid = (Std_ReturnType)E_OK;
        ...
        {
            valid=(Std_ReturnType)E_NOT_OK;            
            (void)Det_ReportError((uint16)MCL_MODULE_ID, 0U, (uint8)MCL_INIT_ID_U8, (uint8)MCL_E_PARAM_CONFIG_U8);
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
    P2VAR(Mcl_DmaTcdAttributesType, AUTOMATIC, MCL_APPL_DATA) config_descriptor_temp = (Mcl_DmaTcdAttributesType*)config_descriptor;
#endif /*MCL_LOGICAL_ADDRESS_USED == STD_ON*/
#endif /*defined MCL_LOGICAL_ADDRESS_USED*/    
#if (MCL_VALIDATE_GLOBAL_CALL == STD_ON)
    VAR(Std_ReturnType, AUTOMATIC)valid;
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
            config_descriptor_temp->u32saddr = Mcl_GetPhysicalAddress(config_descriptor->u32saddr);
            config_descriptor_temp->u32daddr = Mcl_GetPhysicalAddress(config_descriptor->u32daddr);
            /* Call IPW config channel function*/
            Mcl_IPW_DmaConfigChannel(Mcl_pConfig->pMclDmaConfig->pMclDmaHwIpsConfig->pDma_Config, dma_channel, config_descriptor_temp);
#else            
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        if((Std_ReturnType)E_OK == Mcl_ValidateChannelConfigCall(dma_channel, MCL_CONFIG_LINK_CH_ID_U8))
        {
            ...
            else
            {
                ...
                Mcl_IPW_DmaConfigLinkedChannel(Mcl_pConfig->pMclDmaConfig->pMclDmaHwIpsConfig->pDma_Config, dma_channel, config_descriptor_temp, next_channel);
#else                
                                       ...
                                       P2CONST(Mcl_DmaTcdAttributesType, AUTOMATIC, MCL_APPL_CONST) config_descriptor)
{
    ...
    else
    {
        ...
        Mcl_IPW_DmaConfigTcd(pTcdAddress, config_descriptor_temp);
#else        
                                                        ...
                                                        P2VAR(Mcl_DmaTcdType, AUTOMATIC, MCL_APPL_DATA) pNext_tcd)
{
...
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
    P2VAR(Mcl_DmaTcdAttributesType, AUTOMATIC, MCL_APPL_DATA) config_descriptor_temp = (Mcl_DmaTcdAttributesType*)config_descriptor;
    P2VAR(Mcl_DmaTcdType, AUTOMATIC, MCL_APPL_DATA) pNext_tcd_temp;
#endif /*MCL_LOGICAL_ADDRESS_USED == STD_ON*/
#endif /*defined MCL_LOGICAL_ADDRESS_USED*/    
#if (MCL_VALIDATE_GLOBAL_CALL == STD_ON)
    VAR(Std_ReturnType, AUTOMATIC)valid;
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        else
        {
            ...
            config_descriptor_temp->u32saddr = Mcl_GetPhysicalAddress(config_descriptor->u32saddr);
            config_descriptor_temp->u32daddr = Mcl_GetPhysicalAddress(config_descriptor->u32daddr);
            pNext_tcd_temp = (uint32*)Mcl_GetPhysicalAddress((uint32)pNext_tcd);
            /* Call IPW config scatter gather channel function*/
            Mcl_IPW_DmaConfigScatterGatherChannel(Mcl_pConfig->pMclDmaConfig->pMclDmaHwIpsConfig->pDma_Config, dma_channel, config_descriptor_temp, pNext_tcd_temp);
#else
                                             ...
                                             VAR(Mcl_ChannelType, AUTOMATIC) next_channel)
{
    ...
    else
    {
        ...
        Mcl_IPW_DmaConfigLinkedTcd(Mcl_pConfig->pMclDmaConfig->pMclDmaHwIpsConfig->pDma_Config, pTcdAddress, config_descriptor_temp, next_channel);
#else
                                                    ...
                                                    P2VAR(Mcl_DmaTcdType, AUTOMATIC, MCL_APPL_DATA) pNext_tcd)
{
    ...
    else
    {
        ...
        Mcl_IPW_DmaConfigScatterGatherTcd(pTcdAddress, config_descriptor_temp, pNext_tcd_temp);
#else
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        if((Std_ReturnType)E_OK == Mcl_ValidateChannelConfigCall(dma_channel, MCL_CONFIG_SCA_LINK_CH_ID_U8))
        {
            ...
            else
            {
                ...
                pNext_tcd_temp = (uint32*)Mcl_GetPhysicalAddress((uint32)pNext_tcd);
                /* Call IPW config scatter gather channel function*/
                Mcl_IPW_DmaConfigScatterGatherLinkedChannel(Mcl_pConfig->pMclDmaConfig->pMclDmaHwIpsConfig->pDma_Config, dma_channel, config_descriptor_temp, pNext_tcd_temp, next_channel);
#else 
                                                          ...
                                                          VAR(Mcl_ChannelType, AUTOMATIC) next_channel)
{
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        else
        {
            ...
            pNext_tcd_temp = (uint32*)Mcl_GetPhysicalAddress((uint32)pNext_tcd);
            /* Call IPW config scatter gather channel function*/
            Mcl_IPW_DmaConfigScatterGatherLinkedTcd(Mcl_pConfig->pMclDmaConfig->pMclDmaHwIpsConfig->pDma_Config, pTcdAddress, config_descriptor_temp, pNext_tcd_temp, next_channel);
#else 
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
    VAR(uint32, AUTOMATIC) u32Saddr_temp;
#endif /*MCL_LOGICAL_ADDRESS_USED == STD_ON*/
#endif /*defined MCL_LOGICAL_ADDRESS_USED*/
#if (MCL_DEV_ERROR_DETECT == STD_ON)
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
        u32Saddr_temp = Mcl_GetPhysicalAddress(u32Saddr);
        Mcl_IPW_DmaSetSaddr(pTcdAddress, u32Saddr_temp);
#else 
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
    VAR(sint32, AUTOMATIC) s32Dlast_temp;
#endif /*MCL_LOGICAL_ADDRESS_USED == STD_ON*/
#endif /*defined MCL_LOGICAL_ADDRESS_USED*/    
#if (MCL_DEV_ERROR_DETECT == STD_ON)
    if (NULL_PTR == pTcdAddress)
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
        s32Dlast_temp = (sint32)Mcl_GetPhysicalAddress((sint32)s32Dlast);
        Mcl_IPW_DmaSetDlast(pTcdAddress, s32Dlast_temp);
#else 
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
    VAR(uint32, AUTOMATIC) u32Sga_temp;
#endif /*MCL_LOGICAL_ADDRESS_USED == STD_ON*/
#endif /*defined MCL_LOGICAL_ADDRESS_USED*/      
#if (MCL_DEV_ERROR_DETECT == STD_ON)
    if (NULL_PTR == pTcdAddress)
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
        u32Sga_temp = Mcl_GetPhysicalAddress(u32Sga);
        Mcl_IPW_DmaSetSga(pTcdAddress, u32Sga_temp);
#else 
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
    VAR(uint32, AUTOMATIC) u32Daddr_temp;
#endif /*MCL_LOGICAL_ADDRESS_USED == STD_ON*/
#endif /*defined MCL_LOGICAL_ADDRESS_USED*/  
#if (MCL_DEV_ERROR_DETECT == STD_ON)
    if (NULL_PTR == pTcdAddress)
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
        u32Daddr_temp = Mcl_GetPhysicalAddress(u32Daddr);
        Mcl_IPW_DmaSetDaddr(pTcdAddress, u32Daddr_temp);
#else 
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
    VAR(sint32, AUTOMATIC) s32Slast_temp;
#endif /*MCL_LOGICAL_ADDRESS_USED == STD_ON*/
#endif /*defined MCL_LOGICAL_ADDRESS_USED*/     
#if (MCL_DEV_ERROR_DETECT == STD_ON)
    if (NULL_PTR == pTcdAddress)
...
#ifdef MCL_LOGICAL_ADDRESS_USED
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
        s32Slast_temp = (sint32)Mcl_GetPhysicalAddress((sint32)s32Slast);
        Mcl_IPW_DmaSetSlast(pTcdAddress, s32Slast_temp);
#else 
...
LOCAL_INLINE FUNC(uint32, MCL_CODE) Mcl_GetPhysicalAddress(VAR(uint32, AUTOMATIC) logicalAddr)
{
    VAR(uint32, AUTOMATIC) phyisicalAddr= logicalAddr;
    VAR(uint16, AUTOMATIC) i;
    
    for(i=0;i<Mcl_pConfig->pMclLogicalMemoryConfig->NumOfSection;i++)
    {
        if((logicalAddr >= (*(Mcl_pConfig->pMclLogicalMemoryConfig->pSectionConfig))[i].MclLogicalAddrStart) &&
           (logicalAddr <= (*(Mcl_pConfig->pMclLogicalMemoryConfig->pSectionConfig))[i].MclLogicalAddrEnd)
        )
        {
            phyisicalAddr = (logicalAddr - (*(Mcl_pConfig->pMclLogicalMemoryConfig->pSectionConfig))[i].MclLogicalAddrStart) + (*(Mcl_pConfig->pMclLogicalMemoryConfig->pSectionConfig))[i].MclPhysicalAddrStart;
        }
    }
    return phyisicalAddr;
}
