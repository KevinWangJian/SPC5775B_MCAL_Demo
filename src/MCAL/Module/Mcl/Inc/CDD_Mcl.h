...
#define MCL_AR_RELEASE_MINOR_VERSION        0
#define MCL_AR_RELEASE_REVISION_VERSION     3
...
typedef struct
{
    CONST(uint32,  MCL_CONST)                   MclLogicalAddrStart;
    CONST(uint32,  MCL_CONST)                   MclLogicalAddrEnd;
    CONST(uint32,  MCL_CONST)                   MclPhysicalAddrStart;
    CONST(uint32,  MCL_CONST)                   MclPhysicalAddrEnd;
} Mcl_LogicalMemorySectionConfigType;

typedef struct
{
    CONST(uint16,  MCL_CONST)                               NumOfSection;
    CONST(Mcl_LogicalMemorySectionConfigType, MCL_CONST)    (* const pSectionConfig)[];
} Mcl_LogicalMemoryConfigType;
...
#endif
#if (MCL_LOGICAL_ADDRESS_USED == STD_ON)
    P2CONST( Mcl_LogicalMemoryConfigType, MCL_VAR, MCL_APPL_CONST)     pMclLogicalMemoryConfig;
#endif
} Mcl_ConfigType;
