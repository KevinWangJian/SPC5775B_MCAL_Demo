...
/** @violates @ref Gpt_eMios_C_REF_4 Violates MISRA 2004 Required Rule 5.1, The long identifiers are maintained for better readability. */
/** @violates @ref Gpt_eMios_C_REF_6 This is required as per autosar Requirement */
#define GPT_EMIOS_AR_RELEASE_MINOR_VERSION_C        0
/** @violates @ref Gpt_eMios_C_REF_4 Violates MISRA 2004 Required Rule 5.1, The long identifiers are maintained for better readability. */
/** @violates @ref Gpt_eMios_C_REF_6 This is required as per autosar Requirement */
#define GPT_EMIOS_AR_RELEASE_REVISION_VERSION_C     3
#define GPT_EMIOS_SW_MAJOR_VERSION_C                1
#define GPT_EMIOS_SW_MINOR_VERSION_C                0
#define GPT_EMIOS_SW_PATCH_VERSION_C                1
...
* @implements    Gpt_eTimer_ChangeNextTimeoutValue_Activity
*/

FUNC(void, GPT_CODE) Gpt_eMios_ChangeNextTimeoutValue(VAR(uint8, AUTOMATIC) u8HwChannel, VAR(uint16, AUTOMATIC) u16Value)
{
    VAR(uint8, AUTOMATIC) u8ModuleIdx;   /* assigned EMIOS HW module  index */
    VAR(uint8, AUTOMATIC) u8ChannelIdx;  /* assigned EMIOS HW channel index */

    u8ModuleIdx  = EMIOS_MODULE_INDEX_U8(u8HwChannel);
    u8ChannelIdx = EMIOS_CHANNEL_INDEX_U8(u8HwChannel);

    /* Set timeout value to Compare Load Value 1 register*/
    /** @violates @ref Gpt_eMios_C_REF_1 Cast from unsigned long to pointer */
    /** @violates @ref Gpt_eMios_C_REF_7 Violates MISRA 2004 Rule 11.3, Cast from pointer type and integral type.*/
    REG_WRITE32(EMIOS_CADR_ADDR32(u8ModuleIdx, u8ChannelIdx), u16Value);
}
