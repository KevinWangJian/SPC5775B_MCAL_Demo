...
/** @violates @ref GPT_PIT_C_REF_2 Repeated include file  */
#include "Dem.h"
#endif
...
/** @violates @ref GPT_PIT_C_REF_4 MISRA 2004 Rule 1.4, The compiler/linker shall be checked to ensure that 31 character
*   significance and case sensitivity are supported for external identifiers. */
#define GPT_PIT_AR_RELEASE_MINOR_VERSION_C     0
/** @violates @ref GPT_PIT_C_REF_4 MISRA 2004 Rule 1.4, The compiler/linker shall be checked to ensure that 31 character
*   significance and case sensitivity are supported for external identifiers. */
#define GPT_PIT_AR_RELEASE_REVISION_VERSION_C  3
#define GPT_PIT_SW_MAJOR_VERSION_C             1
#define GPT_PIT_SW_MINOR_VERSION_C             0
#define GPT_PIT_SW_PATCH_VERSION_C             1
...
/* Check if source file and SilRegMacros.h are of the same software version */
 ...
 #if ((GPT_PIT_AR_RELEASE_MAJOR_VERSION_C != DEM_AR_RELEASE_MAJOR_VERSION) || \
      (GPT_PIT_AR_RELEASE_MINOR_VERSION_C != DEM_AR_RELEASE_MINOR_VERSION) \
     )
    #error "AutoSar Version Numbers of Gpt_Pit.c and Dem.h are different"
 #endif
#endif
...
/**
* @brief      The function changes the Pit Timer Load Value Register value.
* @details This function:
*          - sets the next timeout value to PIT_RTI_LDVAL.
*
* @param[in]     u8HwChannel        Pit hw channel ID
* @param[in]     u32Value            channel timeout value
* @implements    Gpt_Pit_ChangeNextTimeoutValue_Activity
*/

FUNC(void, GPT_CODE) Gpt_Pit_ChangeNextTimeoutValue(VAR(uint8, AUTOMATIC) u8HwChannel, VAR(uint32, AUTOMATIC) u32Value)
{
    VAR(uint8, AUTOMATIC) u8ModuleIdx;   /* assigned PIT HW module  index */
    VAR(uint8, AUTOMATIC) u8ChannelIdx;  /* assigned PIT HW channel index */

    u8ModuleIdx  = (uint8)PIT_MODULE_INDEX_U32(u8HwChannel);
    u8ChannelIdx = (uint8)PIT_CHANNEL_INDEX_U32(u8HwChannel);
